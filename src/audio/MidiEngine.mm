...
void MidiEngine::startSequencePlayback(int sequenceId) {
    // Simplified playback - in real implementation would use Music Player
    logInfo("MidiEngine: Starting sequence playback (ID " + std::to_string(sequenceId) + ")");

    MidiSequence* sequence = getSequence(sequenceId);
    if (!sequence) return;

    // For now, just play the first few notes as a demo
    for (const auto& track : sequence->tracks) {
        if (!track->notes.empty() && !track->muted) {
            const auto& note = track->notes[0];
