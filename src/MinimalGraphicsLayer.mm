...
@interface WorkingSkiaGraphicsLayer : NSObject
...
@implementation WorkingSkiaGraphicsLayer

- (instancetype)initWithDevice:(id<MTLDevice>)device canvasSize:(CGSize)size {
    self = [super init];
    if (self) {
        NSLog(@"WorkingSkiaGraphicsLayer: Initializing");
        self.device = device;
        ...
        NSLog(@"WorkingSkiaGraphicsLayer: Initialization complete");
    }
    return self;
}
...
- (void)createSkiaSurfaces {
    ...
    NSLog(@"WorkingSkiaGraphicsLayer: Default paint setup complete");
#endif
}
...
#ifdef USE_SKIA
    if (self.backCanvas) {
        NSLog(@"WorkingSkiaGraphicsLayer: Clearing back buffer");
...
#ifdef USE_SKIA
    if (self.backCanvas) {
        NSLog(@"WorkingSkiaGraphicsLayer: Drawing line from (%.1f,%.1f) to (%.1f,%.1f)", x1, y1, x2, y2);
        SkPaint linePaint;
...
#ifdef USE_SKIA
    if (self.backCanvas) {
        NSLog(@"WorkingSkiaGraphicsLayer: Drawing rect at (%.1f,%.1f) size %.1fx%.1f", x, y, w, h);
        SkPaint rectPaint;
...
#ifdef USE_SKIA
    if (self.backCanvas) {
        NSLog(@"WorkingSkiaGraphicsLayer: Filling rect at (%.1f,%.1f) size %.1fx%.1f", x, y, w, h);
        SkPaint fillPaint;
...
#ifdef USE_SKIA
    if (self.backCanvas) {
        NSLog(@"WorkingSkiaGraphicsLayer: Drawing circle at (%.1f,%.1f) radius %.1f", x, y, radius);
        SkPaint circlePaint;
...
- (void)drawTextX:(float)x y:(float)y text:(NSString*)text fontSize:(float)fontSize {
    NSLog(@"WorkingSkiaGraphicsLayer: drawTextX called with text='%@' at (%.1f,%.1f) fontSize=%.1f", text, x, y, fontSize);
    if (!text) return;
    ...
    NSLog(@"WorkingSkiaGraphicsLayer: Queueing GRAPHICS_CMD_DRAW_TEXT command with text='%s'", command.params.drawText.text);
...
- (void)queueCommand:(GraphicsCommand)command {
    NSLog(@"WorkingSkiaGraphicsLayer: Queueing command type %d on thread %@", command.type, [NSThread currentThread]);
    if (command.type == GRAPHICS_CMD_DRAW_TEXT) {
        NSLog(@"WorkingSkiaGraphicsLayer: *** QUEUEING GRAPHICS_CMD_DRAW_TEXT: text='%s' ***", command.params.drawText.text);
    }
    NSData* commandData = [NSData dataWithBytes:&command length:sizeof(GraphicsCommand)];
    [self.queueCondition lock];
    [self.commandQueue addObject:commandData];
    NSLog(@"WorkingSkiaGraphicsLayer: Queue now has %lu commands", (unsigned long)[self.commandQueue count]);
...
- (void)processCommandQueue {
    ...
    for (NSData* commandData in commands) {
        ...
        NSLog(@"WorkingSkiaGraphicsLayer: Processing command type: %d", command.type);
        switch (command.type) {
            case GRAPHICS_CMD_CLEAR:
                NSLog(@"WorkingSkiaGraphicsLayer: Processing GRAPHICS_CMD_CLEAR");
                self.backCanvas->clear(SK_ColorTRANSPARENT);
                break;

            case GRAPHICS_CMD_DRAW_LINE: {
                NSLog(@"WorkingSkiaGraphicsLayer: Processing GRAPHICS_CMD_DRAW_LINE");
            ...
            case GRAPHICS_CMD_DRAW_RECT: {
                NSLog(@"WorkingSkiaGraphicsLayer: Processing GRAPHICS_CMD_DRAW_RECT");
                self.backCanvas->save();
                self.backCanvas->concat(g_currentMatrix);
            ...
            case GRAPHICS_CMD_FILL_RECT: {
                NSLog(@"WorkingSkiaGraphicsLayer: Processing GRAPHICS_CMD_FILL_RECT");
                self.backCanvas->save();
                self.backCanvas->concat(g_currentMatrix);
            ...
            case GRAPHICS_CMD_DRAW_TEXT: {
                NSLog(@"WorkingSkiaGraphicsLayer: *** COMMAND QUEUE PROCESSING GRAPHICS_CMD_DRAW_TEXT ***");
                NSLog(@"WorkingSkiaGraphicsLayer: Processing GRAPHICS_CMD_DRAW_TEXT: text='%s' at (%.1f,%.1f) fontSize=%.1f",
                      command.params.drawText.text, command.params.drawText.x, command.params.drawText.y, command.params.drawText.fontSize);

                self.backCanvas->save();
                self.backCanvas->concat(g_currentMatrix);

                // Draw a background rectangle first to see if positioning is correct
                SkPaint bgPaint;
                bgPaint.setStyle(SkPaint::kFill_Style);
                bgPaint.setColor(SK_ColorBLUE);
                SkRect bgRect = SkRect::MakeXYWH(command.params.drawText.x - 5,
                                                command.params.drawText.y - command.params.drawText.fontSize - 5,
                                                200, command.params.drawText.fontSize + 10);
                self.backCanvas->drawRect(bgRect, bgPaint);
                NSLog(@"WorkingSkiaGraphicsLayer: Drew blue background rectangle");
                ...
                if (!fontMgr) {
                    NSLog(@"WorkingSkiaGraphicsLayer: WARNING - Could not create CoreText font manager");
                    typeface = SkTypeface::MakeEmpty();
                } else {
                    // Try to load PetMe font from assets
                    NSString* fontPath = [[NSBundle mainBundle] pathForResource:@"PetMe" ofType:@"ttf"];
                    if (!fontPath) {
                        ...
                        for (int i = 0; i < 10; i++) {  // Limit search depth to prevent infinite loop
                            ...
                            NSString* petmeFontPath = [assetsPath stringByAppendingPathComponent:@"PetMe.ttf"];

                            if ([[NSFileManager defaultManager] fileExistsAtPath:petmeFontPath]) {
                                fontPath = petmeFontPath;
                                NSLog(@"WorkingSkiaGraphicsLayer: Found PetMe font at: %@", fontPath);
                                break;
                            }
                        ...
                        if (!fontPath) {
                            fontPath = [executableDir stringByAppendingPathComponent:@"../assets/PetMe.ttf"];
                            NSLog(@"WorkingSkiaGraphicsLayer: Using fallback font path: %@", fontPath);
                        }
                    }

                    if (fontPath && [[NSFileManager defaultManager] fileExistsAtPath:fontPath]) {
                        typeface = fontMgr->makeFromFile([fontPath UTF8String]);
                        if (typeface) {
                            NSLog(@"WorkingSkiaGraphicsLayer: Loaded PetMe font from %@", fontPath);
                        }
                    }

                    // Fallback to system font if PetMe didn't work
                    if (!typeface) {
                        // Try to match system font by name
                        typeface = fontMgr->legacyMakeTypeface("Helvetica", SkFontStyle::Normal());
                        if (typeface) {
                            NSLog(@"WorkingSkiaGraphicsLayer: Loaded Helvetica system font");
                        }
                    }

                    // Another fallback - try Arial
                    if (!typeface) {
                        typeface = fontMgr->legacyMakeTypeface("Arial", SkFontStyle::Normal());
                        if (typeface) {
                            NSLog(@"WorkingSkiaGraphicsLayer: Loaded Arial system font");
                        }
                    }
                    ...
                    if (!typeface) {
                        typeface = SkTypeface::MakeEmpty();
                        NSLog(@"WorkingSkiaGraphicsLayer: WARNING - Using empty typeface, no text will be visible");
                    }
                }

                font.setTypeface(typeface);
                NSLog(@"WorkingSkiaGraphicsLayer: Successfully created typeface: %s",
                      typeface ? "valid" : "null");

                // Set paint style for text - use bright colors
                SkPaint textPaint;
                textPaint.setStyle(SkPaint::kFill_Style);
                textPaint.setAntiAlias(true);
                textPaint.setColor(SK_ColorWHITE); // Force white color
                ...
                NSLog(@"WorkingSkiaGraphicsLayer: About to draw Skia text: '%s' at (%.1f,%.1f) with fontSize %.1f",
                ...
                self.backCanvas->drawString(skString,
                    ...
                    font, textPaint);
                NSLog(@"WorkingSkiaGraphicsLayer: Skia drawString completed");
...
- (void)swapBuffers {
#ifdef USE_SKIA
    NSLog(@"WorkingSkiaGraphicsLayer: Swapping front and back buffers");
    ...
    NSLog(@"WorkingSkiaGraphicsLayer: Buffer swap complete");
#endif
}
...
static WorkingSkiaGraphicsLayer* g_workingLayer = nil;

extern "C" {
    void minimal_graphics_layer_init(void* device, float width, float height) {
        NSLog(@"WorkingSkiaGraphicsLayer: init called with device=%p size=%.0fx%.0f", device, width, height);
        id<MTLDevice> metalDevice = (__bridge id<MTLDevice>)device;

        g_workingLayer = [[WorkingSkiaGraphicsLayer alloc] initWithDevice:metalDevice canvasSize:CGSizeMake(width, height)];
        g_minimalGraphicsInitialized = (g_workingLayer != nil);

        NSLog(@"WorkingSkiaGraphicsLayer: init complete");
    }

    void minimal_graphics_layer_clear() {
        NSLog(@"C API: minimal_graphics_layer_clear called on thread %@", [NSThread currentThread]);
        if (g_workingLayer) {
            GraphicsCommand command;
            memset(&command, 0, sizeof(command));
            command.type = GRAPHICS_CMD_CLEAR;
            [g_workingLayer queueCommand:command];
        }
    }

    void minimal_graphics_layer_set_color(float r, float g, float b, float a) {
        NSLog(@"minimal_graphics_layer_set_color: RGBA(%.2f, %.2f, %.2f, %.2f) - storing for next draw", r, g, b, a);
        // Store color in layer for next draw commands - no need to queue this
        if (g_workingLayer) {
            [g_workingLayer setColor:r g:g b:b a:a];
        }
    }
    ...
    void minimal_graphics_layer_draw_line(float x1, float y1, float x2, float y2) {
        NSLog(@"minimal_graphics_layer_draw_line: (%.1f,%.1f) to (%.1f,%.1f)", x1, y1, x2, y2);
        if (g_workingLayer) {
            GraphicsCommand command;
            memset(&command, 0, sizeof(command));
            command.type = GRAPHICS_CMD_DRAW_LINE;
            command.r = g_workingLayer.inkColor.x;
            command.g = g_workingLayer.inkColor.y;
            command.b = g_workingLayer.inkColor.z;
            command.a = g_workingLayer.inkColor.w;
    ...
    void minimal_graphics_layer_draw_text(float x, float y, const char* text, float fontSize) {
        NSLog(@"minimal_graphics_layer_draw_text: called with text='%s' at (%.1f,%.1f) fontSize=%.1f", text ? text : "NULL", x, y, fontSize);
        if (g_workingLayer && text) {
            GraphicsCommand command;
            memset(&command, 0, sizeof(command));
            command.type = GRAPHICS_CMD_DRAW_TEXT;
            command.r = g_workingLayer.inkColor.x;
            ...
            command.params.drawText.text[sizeof(command.params.drawText.text) - 1] = '\0';
            NSLog(@"minimal_graphics_layer_draw_text: Calling queueCommand with ink color (%.2f,%.2f,%.2f,%.2f)",
                  command.r, command.g, command.b, command.a);
            [g_workingLayer queueCommand:command];
        } else {
            NSLog(@"minimal_graphics_layer_draw_text: ERROR - g_workingLayer=%p text=%p", g_workingLayer, text);
        }
    }
    ...
    void minimal_graphics_layer_render(void* encoder, float width, float height) {
        if (g_workingLayer) {
            ...
            [g_workingLayer renderWithEncoder:metalEncoder viewport:CGSizeMake(width, height)];
        }
    }

    void minimal_graphics_layer_swap() {
        NSLog(@"C API: minimal_graphics_layer_swap called on thread %@", [NSThread currentThread]);
        if (g_workingLayer) {
            GraphicsCommand command;
            memset(&command, 0, sizeof(command));
    ...
    void minimal_graphics_layer_cleanup() {
        NSLog(@"WorkingSkiaGraphicsLayer: cleanup called");
        g_workingLayer = nil;
        g_minimalGraphicsInitialized = false;
        NSLog(@"WorkingSkiaGraphicsLayer: cleanup complete");
    }

    void minimal_graphics_layer_set_ink(float r, float g, float b, float a) {
        NSLog(@"minimal_graphics_layer_set_ink: RGBA(%.2f, %.2f, %.2f, %.2f)", r, g, b, a);
        if (g_workingLayer) {
            g_workingLayer.inkColor = simd_make_float4(r, g, b, a);
        }
    }

    void minimal_graphics_layer_set_paper(float r, float g, float b, float a) {
        NSLog(@"minimal_graphics_layer_set_paper: RGBA(%.2f, %.2f, %.2f, %.2f)", r, g, b, a);
        if (g_workingLayer) {
            g_workingLayer.paperColor = simd_make_float4(r, g, b, a);
        }
    }

    void minimal_graphics_layer_show() {
        NSLog(@"minimal_graphics_layer_show: Graphics layer visible");
        if (g_workingLayer) {
            g_workingLayer.graphicsVisible = YES;
        }
    }

    void minimal_graphics_layer_hide() {
        NSLog(@"minimal_graphics_layer_hide: Graphics layer hidden");
        if (g_workingLayer) {
            g_workingLayer.graphicsVisible = NO;
        }
    }
    ...
    bool minimal_graphics_layer_load_image(uint16_t imageId, const char* filename) {
        if (!g_workingLayer) {
            NSLog(@"minimal_graphics_layer_load_image: No working layer");
            return false;
        }
        ...
        if (!data) {
            NSLog(@"minimal_graphics_layer_load_image: Failed to load file %s", filename);
            return false;
        }
        ...
        if (!image) {
            NSLog(@"minimal_graphics_layer_load_image: Failed to decode image %s", filename);
            return false;
        }
        ...
        NSValue* imageValue = [NSValue valueWithPointer:image.get()];
        g_workingLayer.loadedImages[key] = imageValue;

        NSLog(@"minimal_graphics_layer_load_image: Loaded image %d (%s) size %dx%d",
    ...
    bool minimal_graphics_layer_get_image_size(uint16_t imageId, int* width, int* height) {
        if (g_workingLayer && width && height) {
            auto it = g_globalImages.find(imageId);
            if (it != g_globalImages.end()) {
                sk_sp<SkImage> image = it->second;
                *width = image->width();
    ...
    void minimal_graphics_layer_draw_image_rect(uint16_t imageId, float srcX, float srcY, float srcW, float srcH,
    ...
    bool minimal_graphics_layer_read_pixels(uint16_t imageId, int x, int y, int width, int height, uint8_t** outPixels) {
        if (!g_workingLayer || !outPixels) return false;

        if (imageId == 0) {
            // Read from screen (back buffer)
            if (g_workingLayer.backCanvas) {
                SkImageInfo info = SkImageInfo::Make(width, height, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
                size_t rowBytes = width * 4;
                uint8_t* pixels = (uint8_t*)malloc(height * rowBytes);
                if (pixels) {
                    if (g_workingLayer.backCanvas->readPixels(info, pixels, rowBytes, x, y)) {
                        *outPixels = pixels;
                        return true;
                    } else {
                        free(pixels);
    ...
    bool minimal_graphics_layer_write_pixels(uint16_t imageId, int x, int y, int width, int height, const uint8_t* pixels) {
        if (!g_workingLayer || !pixels) return false;
    ...
    bool minimal_graphics_layer_create_sprite_from_canvas(uint16_t spriteId, int width, int height) {
        if (!g_workingLayer) return false;
        ...
        if ([g_workingLayer getCanvas]->readPixels(info, pixels, rowBytes, 0, 0)) {
    ...
    bool minimal_graphics_layer_begin_sprite_render(uint16_t spriteId, int width, int height) {
    ...
    bool minimal_graphics_layer_end_sprite_render(uint16_t spriteId) {
        ...
        [g_workingLayer processCommandQueue];
    ...
    bool minimal_graphics_layer_is_sprite_rendering() {
        return g_creatingSprite;
    }

    // Tile rendering functions
    bool minimal_graphics_layer_begin_tile_render(uint16_t tileId, int width, int height) {
    ...
    bool minimal_graphics_layer_end_tile_render(uint16_t tileId) {
    ...
    bool minimal_graphics_layer_is_tile_rendering() {
    ...
    bool minimal_graphics_layer_is_ui_rendering() {
